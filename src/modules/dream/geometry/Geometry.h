...
public:
  ...
  uint32_t getPixel(uint8_t Sector, uint8_t Sumo, uint8_t Cassette,
    ...
    if (not isValidParameter(Sector, Sumo, Cassette, Counter, Wire, Strip)) {
      return 0;
    }
  ...
  uint16_t getX(uint8_t Sector, uint8_t Sumo,
...
private:
  ...
  bool isValidParameter(uint8_t Sector, uint8_t Sumo, uint8_t Cassette,
                    uint8_t Counter, uint8_t Wire, uint8_t Strip) {
    if (Sector > MaxSector) {
