...
/// \file
///
/// \brief VMM instrument base plugin
///
//===----------------------------------------------------------------------===//
...
const char *classname = "VMM detector with ESS readout";

VMMBase::VMMBase(BaseSettings const &settings) : Detector(settings) {
  ...
  std::function<void()> processingFunc = [this]() {
    VMMBase::processing_thread();
  };
  Detector::AddThreadFunction(processingFunc, "processing");

  XTRACE(INIT, ALW, "Creating %d VMM Rx ringbuffers of size %d",
         EthernetBufferMaxEntries, EthernetBufferSize);
}

void VMMBase::processing_thread() {

  // Event producer
  if (EFUSettings.KafkaTopic == "") {
    XTRACE(INIT, ALW, "Setting defailt Kafka topic to vmm_detector");
    EFUSettings.KafkaTopic = "vmm_detector";
  }
  ...
  Producer MonitorProducer(EFUSettings.KafkaBroker, "vmm_debug",
                           KafkaCfg.CfgParms);
  auto ProduceMonitor = [&MonitorProducer](auto DataBuffer, auto Timestamp) {
    MonitorProducer.produce(DataBuffer, Timestamp);
  };

  Serializer = new EV44Serializer(KafkaBufferSize, "vmm", Produce);
  VMMInstrument VMM(Counters, EFUSettings, Serializer);

  HistogramSerializer ADCHistSerializer(VMM.ADCHist.needed_buffer_size(),
                                        "VMM");
  ...
  while (runThreads) {
    if (InputFifo.pop(DataIndex)) { // There is data in the FIFO - do processing
      ...
      Res = VMM.VMMParser.parse(VMM.ESSReadoutParser.Packet);
      Counters.TimeStats = VMM.ESSReadoutParser.Packet.Time.Stats;
      ...
      for (auto &builder : VMM.builders) {
        ...
        if (VMM.Conf.VMMFileParameters.SplitMultiEvents) {
    ...
    if (ProduceTimer.timeout()) {
      ...
      if (!VMM.ADCHist.isEmpty()) {
        XTRACE(PROCESS, DEB, "Sending ADC histogram for %zu readouts",
               VMM.ADCHist.hitCount());
        ADCHistSerializer.produce(VMM.ADCHist);
        VMM.ADCHist.clear();
      }
      // if (!VMM.TDCHist.isEmpty()) {
      //   XTRACE(PROCESS, DEB, "Sending TDC histogram for %zu readouts",
      //      VMM.TDCHist.hitCount());
      //   TDCHistSerializer.produce(VMM.TDCHist);
      //   VMM.TDCHist.clear();
      // }
    }
  }
