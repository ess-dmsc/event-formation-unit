...
class VMMBaseStandIn : public VMM::VMMBase {
public:
  VMMBaseStandIn(BaseSettings Settings)
      : VMM::VMMBase(Settings){};
  ~VMMBaseStandIn() = default;
  using Detector::Threads;
  using VMM::VMMBase::Counters;
};

class VMMBaseTest : public ::testing::Test {
public:
  void SetUp() override {
...
TEST_F(VMMBaseTest, Constructor) {
  VMMBaseStandIn Readout(Settings);
  EXPECT_EQ(Readout.ITCounters.RxPackets, 0);
  EXPECT_EQ(Readout.Counters.VMMStats.Readouts, 0);
}

TEST_F(VMMBaseTest, DataReceive) {
  VMMBaseStandIn Readout(Settings);
...
TEST_F(VMMBaseTest, DataReceiveBadHeader) {
  VMMBaseStandIn Readout(Settings);
  Readout.startThreads();
  std::chrono::duration<std::int64_t, std::milli> SleepTime{400};
  std::this_thread::sleep_for(SleepTime);
