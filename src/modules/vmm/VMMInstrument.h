...
/// formation
/// Its functions are called from the main prcessing loop in VMMBase
//===----------------------------------------------------------------------===//
...
class VMMInstrument {
public:
  /// \brief 'create' the VMM instrument
  ...
  /// \brief initialise the serializer. This is used both in VMMInstrument
  // and VMMBase. Called from VMMBase
  void setSerializer(EV44Serializer *serializer) { Serializer = serializer; }

public:
  /// \brief Stuff that 'ties' VMM together
