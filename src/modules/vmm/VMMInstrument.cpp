...
/// \file
///
/// \brief VMMInstrument is responsible for readout validation and event
/// formation
///
//===----------------------------------------------------------------------===//
...
void VMMInstrument::loadConfigAndCalib() {
  XTRACE(INIT, ALW, "Loading configuration file %s",
         Settings.ConfigFile.c_str());
  Conf = Config("VMM", Settings.ConfigFile);
...
void VMMInstrument::generateEvents(std::vector<Event> &Events) {
  ...
  for (const auto &e : Events) {
    ...
    if (Conf.WireGapCheck) {
      if (e.ClusterB.hasGap(Conf.VMMFileParameters.MaxGapWire)) {
    ...
    if (Conf.StripGapCheck) {
      if (e.ClusterA.hasGap(Conf.VMMFileParameters.MaxGapStrip)) {
