...
class LETGeometryTest : public TestBase {
protected:
  LETGeometry Geom;
  uint16_t VMM0{0};
  uint16_t VMM1{1};
...
TEST_F(LETGeometryTest, DefaultCSPEC) {
  ASSERT_TRUE(Geom.isWire(VMM1));
  ASSERT_TRUE(Geom.isGrid(VMM0));
}

TEST_F(LETGeometryTest, XAndZCoordinateCalculations) {
  // xAndzCoord takes RingID, FENID, HybridID, VMMID, Channel, XOffset, Rotated

  // Vessel 0, very first xAndzCoord position
  ASSERT_EQ(Geom.xAndzCoord(1, 0, 1, 0, 33, 0, false), 0);
...
TEST_F(LETGeometryTest, InvalidXAndZCoordinates) {
  // invalid HybridID
...
TEST_F(LETGeometryTest, InvalidYCoordinates) {
