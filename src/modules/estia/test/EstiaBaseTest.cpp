...
/// \file
///
/// \brief Unit tests for EstiaBase
///
//===----------------------------------------------------------------------===//
...
class EstiaBaseTest : public ::testing::Test {
public:
  BaseSettings Settings;

  void SetUp() override {
    Settings.ConfigFile = ESTIA_FULL;
    Settings.KafkaTopic = "estia_detector";
...
TEST_F(EstiaBaseTest, Constructor) {
  Estia::EstiaBase Readout(Settings);
  EXPECT_EQ(Readout.ITCounters.RxPackets, 0);
  EXPECT_EQ(Readout.Counters.VMMStats.Readouts, 0);
}

TEST_F(EstiaBaseTest, DataReceive) {
  Estia::EstiaBase Readout(Settings);
  ...
  // Estia has two detectors with 48 cassettes each, thus 96 event builders
  // so match attempt count increases by 96 for each packet received
  EXPECT_EQ(Readout.Counters.MatcherStats.MatchAttemptCount, 96);
  Readout.stopThreads();
}

TEST_F(EstiaBaseTest, DataReceiveBadHeader) {
  Estia::EstiaBase Readout(Settings);
...
TEST_F(EstiaBaseTest, EmulateFIFOError) {
  Estia::EstiaBase Readout(Settings);
