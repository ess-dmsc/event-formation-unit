...
EstiaInstrument::EstiaInstrument(struct Counters &counters,
  ...
  if (!Settings.DumpFilePrefix.empty()) {
    std::string DumpFileName =
        Settings.DumpFilePrefix + "estia_" + timeString();
...
void EstiaInstrument::loadConfigAndCalib() {
  XTRACE(INIT, ALW, "Loading configuration file %s",
         Settings.ConfigFile.c_str());
  Conf = Config("Estia", Settings.ConfigFile);
...
void EstiaInstrument::processReadouts(void) {
  XTRACE(DATA, DEB,
         "\n================== NEW PACKET =====================\n\n");
  ...
  for (const auto &readout : VMMParser.ResultCluster) {
    ...
    if (!Conf.getHybrid(Ring, readout.FENId, HybridId).Initialised) {
     ...
     uint64_t EventTimeNS =
      ...
      uint16_t XCoord =
          Geom.xCoord(Hybrid.XOffset, readout.Hybrid*2, readout.Position0);
      XTRACE(DATA, INF,
...
void EstiaInstrument::generateEvents(std::vector<Event> &Events) {
  ESSReadout::ESSReferenceTime &TimeRef = ESSReadoutParser.Packet.Time;
