...
/// \file
///
/// \brief Estia instrument base plugin
///
//===----------------------------------------------------------------------===//
...
const char *classname = "Estia detector with ESS readout";

EstiaBase::EstiaBase(BaseSettings const &settings) : Detector(settings) {
  ...
  std::function<void()> processingFunc = [this]() {
    EstiaBase::processing_thread();
  };
  Detector::AddThreadFunction(processingFunc, "processing");

  XTRACE(INIT, ALW, "Creating %d Estia Rx ringbuffers of size %d",
         EthernetBufferMaxEntries, EthernetBufferSize);
}

void EstiaBase::processing_thread() {
  ...
  Serializer = new EV44Serializer(KafkaBufferSize, "estia", Produce);
  ...
  MonitorSerializer = new AR51Serializer("estia", ProduceMonitor);

  EstiaInstrument Estia(Counters, EFUSettings, Serializer);
  ...
  while (runThreads) {
    if (InputFifo.pop(DataIndex)) { // There is data in the FIFO - do processing
      ...
      auto Res = Estia.ESSReadoutParser.validate(DataPtr, DataLen,
                                                 ESSReadout::Parser::ESTIA);
      Counters.ReadoutStats = Estia.ESSReadoutParser.Stats;
      ...
      Res = Estia.VMMParser.parse(Estia.ESSReadoutParser.Packet);
      Counters.TimeStats = Estia.ESSReadoutParser.Packet.Time.Stats;
