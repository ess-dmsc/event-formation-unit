...
TEST_F(PanelGeometryTest, CanonicalCornersWithOffset) {
  ...
  const int StrawsPerCol{TubesXY8*7};
  ASSERT_EQ(PG.getGlobalStrawId(0, 0, 0), 0 * StrawsPerCol + Offset10000); // Top left
  ASSERT_EQ(PG.getGlobalStrawId(0, 3, 0), 3 * StrawsPerCol + Offset10000); // Top right
  ASSERT_EQ(PG.getGlobalStrawId(3, 4, 6), StrawsPerCol - 1 + Offset10000); // Bottom left
  ASSERT_EQ(PG.getGlobalStrawId(3, 7, 6), 4 * StrawsPerCol - 1 + Offset10000); // Bottom right
}
