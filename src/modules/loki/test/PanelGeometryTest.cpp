...
TEST_F(PanelGeometryTest, Constructor) {
  ...
  uint32_t Straw = PG.getGlobalUnitId(TubeGroup, LocalTube, LocalStraw);
  ASSERT_EQ(Straw, 0);
}
...
TEST_F(PanelGeometryTest, CanonicalCorners) {
  ...
  ASSERT_EQ(PG.getGlobalUnitId(0, 0, 0), 0 * StrawsPerCol);     // Top left
  ASSERT_EQ(PG.getGlobalUnitId(0, 3, 0), 3 * StrawsPerCol);     // Top right
  ASSERT_EQ(PG.getGlobalUnitId(3, 4, 6), StrawsPerCol - 1);     // Bottom left
  ASSERT_EQ(PG.getGlobalUnitId(3, 7, 6), 4 * StrawsPerCol - 1); // Bottom right
}
...
TEST_F(PanelGeometryTest, InvalidGeometry) {
  ...
  ASSERT_EQ(PG.getGlobalUnitId(1, 0, 7), 0xffffffff); // Straw too large
}
