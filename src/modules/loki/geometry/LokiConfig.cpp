...
// Add the missing constructor implementation
LokiConfig::LokiConfig(const std::string &ConfigFile) : Config(ConfigFile) {
...
void LokiConfig::parseConfig() {
  ...
  if (InstrumentName != "loki") {
    LOG(INIT, Sev::Error, "Invalid instrument name ({}) for loki",
        InstrumentName);
  ...
  try {
    ...
    for (const auto &elt : Configs) {
      ...
      XTRACE(INIT, ALW, "Ring %2d, Bank %2d, FENs %2d, FENOffset %2d", Ring,
             Bank, FENs, FENOffset);
