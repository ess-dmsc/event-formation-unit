...
TEST_F(FreiaGeometryTest, BoundaryTests) {
  ...
  EXPECT_EQ(Geom.getVmmGeometryCounters().InvalidXCoord, 1); // Still 1 from the channel validation above
  EXPECT_EQ(Geom.getVmmGeometryCounters().InvalidYCoord, 1);
  ...
  EXPECT_EQ(Geom.getVmmGeometryCounters().InvalidXCoord, 2); // Incremented due to overflow
  EXPECT_EQ(Geom.getVmmGeometryCounters().InvalidYCoord, 1);
