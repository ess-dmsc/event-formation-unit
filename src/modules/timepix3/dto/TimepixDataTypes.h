 ...
 * 
 * This structure contains two fields: pulseTimeInEpochNs and tdcClockInPixelTime,
 * representing the pulse time in epoch nanoseconds and the TDC clock in pixel time, respectively.
 */
 ...
 * 
 * This struct contains information about an EVR data event, including the counter value,
 * the pulse time in epoch nanoseconds, and the arrival timestamp.
 */
struct EVRDataEvent {
  const uint32_t counter; /**< The counter value. */
  const uint64_t pulseTimeInEpochNs; /**< The pulse time in epoch nanoseconds. */
  const std::chrono::high_resolution_clock::time_point arrivalTimestamp; /**< The arrival timestamp. */
 ...
 * 
 * This structure holds the EVR readout data, including various fields such as type, counter, pulse time, and previous pulse time.
 * It also provides an equality comparison operator for comparing two EVRReadout objects.
 */
struct EVRReadout {
  ...
  const uint32_t prevPulseTimeNanoSeconds; /**< The previous pulse time in nanoseconds. */
 ...
 * 
 * This struct stores information about a TDC readout, including the type, counter value,
 * timestamp, and stamp. It also provides an equality comparison operator for comparing
 * two TDCReadout objects.
 */
struct TDCReadout {
