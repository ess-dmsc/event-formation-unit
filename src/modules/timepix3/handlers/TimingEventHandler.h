...
namespace Timepix3 {
  ...
  /// DataEventObserver for TDCReadout and EVRReadout, and as a
  /// DataEventObservable for ESSGlobalTimeStamp.
  ///
  /// The class maintains a threshold value for the timing difference between
  /// TDC and EVR events, and provides a method to check if the last timing
  /// difference is lower than the threshold.
  ///
  /// The class also stores the last TDCDataEvent and EVRDataEvent received, and
  /// provides methods to apply data to these events.
  ///
  /// \note The threshold value is currently hard-coded and can be moved to a
  ...
  class TimingEventHandler
   ...
   private:
    ...
    const std::chrono::milliseconds THRESHOLD_MS =
        std::chrono::milliseconds(20);

    Counters &statCounters;

    const std::chrono::nanoseconds FrequencyPeriodNs;

    unique_ptr<timepixDTO::TDCDataEvent> lastTDCData;
    unique_ptr<timepixDTO::EVRDataEvent> lastEVRData;

    uint32_t tdcRepetitionFrequency{
        static_cast<uint32_t>(FrequencyPeriodNs.count())};
   ...
   public:
    ...
    ///
    /// \param tdcReadout The TDCReadout object containing the data to be
    /// applied.
    ///
    ...
    ///
    /// \param evrReadout The EVRReadout object containing the data to be
    /// applied.
    ///
