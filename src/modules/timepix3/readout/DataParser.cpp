...
int DataParser::parse(const char *Buffer, unsigned int Size) {
  ...
  //information. If they are 24 bits but not type = 1, then it is a camera packet 
  if(Size == 24){
  ...
  while (BytesLeft) {
    ...
    if (ReadoutType == 11) {
      ...
      // toa formula is based on information from the timepix3 manual provided with the camera
      ...
      //LastTDCTime is the latest seen time from a TDC readout, and this counter is used to
      //see how strictly in order the pixel readouts are in relation to this. Pixel readouts
      //from before the last TDC time belong to the previous pulse, and may need to be treated 
      //differently
      if(toa < LastTDCTime){
    ...
    // TDC readout type, indicating when the camera received a TDC pulse. In the ESS setup, this should
    // correspond to an EVR pulse, indicating the start of a new pulse.
      ...
      // TDC readouts can belong to one of two channels, and can either indicate the rising
      // or the falling edge of the signal. The camera setup will determine which of these
      // are sent.
      if (Data.Type == 15) {
    ...
    // global timestamps are a readout allowing for longer periods of time to be recorded before "overflowing"
    // to 0. This readout type isn't expected to be used in production, currently.
      ...
      // we sometimes see packet type 7 here, which accompanies a lot of control signals
