...
int main(const int argc, char *argv[]) {
  ...
  UdpConnections.emplace_back(&AdcBox1);
  TriggerTime_UdpHeartBeat.emplace_back();
  UdpConnections.emplace_back(&AdcBox2);
  TriggerTime_UdpHeartBeat.emplace_back();
  ...
  std::chrono::duration<size_t, std::nano> PrintStatsDeltaTime(
      5'000'000'000ull);
  ...
  while (RunLoop) {
    ...
    // PrintStats
    {
      ...
      if (TriggerTime <= TimeNow) {
        TriggerTime = TimeNow + PrintStatsDeltaTime;
        if (runTriggers) {
          PrintStats();
        }
      }
    ...
    for (int32_t i = 0, count = UdpConnections.size(); i < count; i++) {
      ...
      if (TriggerTime <= TimeNow) {
        TriggerTime = TimeNow + self.heartBeatIntervalTime;
        if (runTriggers) {
