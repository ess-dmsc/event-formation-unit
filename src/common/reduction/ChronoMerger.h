...
public:
  ...
  ///        the observed latency of the readout system, but in practice could
  ///        be higher depending on buffer sizes in clustering pipelines.
  ...
  /// \param module Identifies the pipeline from which events originate. All
  /// events
  ///               in merged queue must originate from the same pipeline for
  ///               promised guarantees to hold.
  /// \param events Queue of events that will be spliced. The original queue
  /// will be
  ///               rendered empty.
  void insert(size_t module, std::list<NeutronEvent> &events);

  /// \brief Forcibly syncs up the time horizons of two pipelines, picking the
  /// later
  ///        of the two horizons. This is to be used when external is sufficient
  ///        to establish that one pipeline's time progress implies the same for
  ///        another. This can be used for partial "flushing" of data.
  /// \param module1 id of first pipeline
  /// \param module2 id of second pipeline
  void sync_up(size_t module1, size_t module2);

  /// \brief Sorts the queue chronologically. This should be performed at some
  /// point
  ///        after inserting some data and before attempting to pop any off the
  ///        queue.
  void sort();
  ...
  /// \returns the global time horizon for all tracked pipelines. The horizon
  /// for each
  ///          pipeline is the latest time-point seen in among received events.
  ///          The global horizon is the earliest of these pipeline horizons,
  ///          i.e. only as far as the most lagging pipeline can see.
  uint64_t horizon() const;

  /// \returns true if pipeline is non-empty and earliest event in queue is
  /// outside
  ///          the maximum latency window in relation to the global time
  ///          horizon.
