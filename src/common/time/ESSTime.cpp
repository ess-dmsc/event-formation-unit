...
std::string toString(TimeDurationNano TimeStamp) {
  ...
  // Convert nanoseconds to system_clock's duration (microseconds on macOS,
  // nanoseconds on Linux)
  system_clock::time_point tp{
      duration_cast<system_clock::duration>(nanoseconds{TimeStamp})};
...
tof_t ESSReferenceTime::getTOF(const ESSTime &eventTime, uint32_t DelayNS) {
  ...
  if (timeval < TimeInNS) {
    XTRACE(EVENT, WAR,
           "TOF negative:      High: 0x%08x, Low: 0x%08x, PT/ns: %" PRIu64
           " (%s), Prev PT/ns: %" PRIu64 " (%s)",
           eventTime.getTimeHigh(), eventTime.getTimeLow(), timeval.count(),
           toString(timeval).c_str(), TimeInNS.count(),
           toString(TimeInNS).c_str());
  ...
  if ((timeval - TimeInNS) > MaxTOF) {
    XTRACE(EVENT, WAR,
           "High TOF:          High: 0x%08x, Low: 0x%08x, PT/ns: %" PRIu64
           " (%s), Prev PT/ns: %" PRIu64 " (%s)",
           eventTime.getTimeHigh(), eventTime.getTimeLow(), timeval.count(),
           toString(timeval).c_str(), TimeInNS.count(),
           toString(TimeInNS).c_str());
...
tof_t ESSReferenceTime::getPrevTOF(const ESSTime &eventTime, uint32_t DelayNS) {
  TimeDurationNano timeval = eventTime.toNS() + TimeDurationNano(DelayNS);

  if (timeval < PrevTimeInNS) {
    XTRACE(EVENT, WAR,
           "Prev TOF negative: High: 0x%08x, Low: 0x%08x, PT/ns: %" PRIu64
           " (%s), Prev PT/ns: %" PRIu64 " (%s)",
           eventTime.getTimeHigh(), eventTime.getTimeLow(), timeval.count(),
           toString(timeval).c_str(), PrevTimeInNS.count(),
           toString(PrevTimeInNS).c_str());
  ...
  if ((timeval - PrevTimeInNS) > MaxTOF) {
    XTRACE(EVENT, WAR,
           "High Prev TOF:     High: 0x%08x, Low: 0x%08x, PT/ns: %" PRIu64
           " (%s), Prev PT/ns: %" PRIu64 " (%s)",
           eventTime.getTimeHigh(), eventTime.getTimeLow(), timeval.count(),
           toString(timeval).c_str(), PrevTimeInNS.count(),
           toString(PrevTimeInNS).c_str());
