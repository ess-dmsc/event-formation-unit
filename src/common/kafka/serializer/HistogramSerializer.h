...
public:
  ...
  HistogramSerializer(
      ...
      const essmath::VectorAggregationFunc<T> AggFunc = essmath::SUM_AGG_FUNC<T>,
      ...
      : AbstractSerializer(Callback, Stats), Topic(Topic), Source(Source),
        ...
        AggregateFunction(AggFunc), BinningStrategy(Strategy) {
...
private:
  ...
  void serialize() {
    ...
    auto XAxis = (da00_flatbuffers::Variable("time", {"t"},
                                             {static_cast<time_t>(BinCount)})
                      .unit(TimeUnit)
                      .data(XAxisValues));

    auto YAxis = (da00_flatbuffers::Variable(Name, {"a.u."},
                                             {static_cast<time_t>(BinCount)})
                      .unit(Unit)
                      .data(AggregatedBins));

    const auto dataarray = DataArray("histogram_sender", {XAxis, YAxis});
