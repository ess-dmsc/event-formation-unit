...
protected:
  ...
  ESSGeometry Geometry{UdderTestImage.ImageWidth, UdderTestImage.ImageHeight, 1,
                       1};
};
...
TEST_F(TestImageUdderTest, FirstFewPixels) {
  auto pixel = UdderTestImage.getPixel(UdderTestImage.ImageWidth,
                                       UdderTestImage.ImageHeight, &Geometry);
  ASSERT_EQ(pixel, 123);

  pixel = UdderTestImage.getPixel(UdderTestImage.ImageWidth,
                                  UdderTestImage.ImageHeight, &Geometry);
  ASSERT_EQ(pixel, 124);
}
...
TEST_F(TestImageUdderTest, CachePixelsWrap) {
  ...
  for (uint32_t i = 0; i < UdderTestImage.getNumberOfCachedPixels(); i++) {
    LastPixel = UdderTestImage.getPixel(UdderTestImage.ImageWidth,
                                        UdderTestImage.ImageHeight, &Geometry);
