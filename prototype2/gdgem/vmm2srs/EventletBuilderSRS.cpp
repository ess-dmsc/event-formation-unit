    ...
    , geometry_interpreter_(geometry_interpreter)
{
  ...
  std::string startTime = cStartTime;
        ...
        dprintf(fd, "%s\n", s);
        dprintf(fd, "# fec, chip_id, frame counter, srs timestamp, channel, bcid, tdc, adc, overthreshold\n");
#endif
}
...
#ifdef DUMPTOFILE
    dprintf(fd, "%2d, %2d, %u, %u, %2d, %d, %d, %d\n", 1, chip_id, parser_.srshdr.fc, parser_.srshdr.time,
