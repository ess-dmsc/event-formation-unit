...
ReaderAPV::ReaderAPV(std::string filename) {
  ...
  try {
    ...
    if ((shape.size() != 2) || (shape[1] != 4))
      throw std::runtime_error("<ReaderAPV> wrong dimensions for RawVMM/points");

    total_ = shape[0] - 1;
    std::cout << "<ReaderAPV> opened " << filename << " with " << total_
              << " events, should produce ~" << (total_ / max_in_buf_)
              << " buffers\n";
  } catch (std::exception &e) {
    std::cout << "<ReaderAPV> failed to open " << filename << "\n"
              << hdf5::error::print_nested(e, 1) << std::endl;
  }
}

size_t ReaderAPV::read(char *buf) {
  ...
  if (slab_.block()[0] > 0) {
    try {
      ...
      std::cout << "<ReaderAPV> failed to read slab ("
