...
protected:
  ...
  Cluster *cluster;
  virtual void SetUp() { cluster = new Cluster(); }
  virtual void TearDown() { delete cluster; }
};

TEST_F(ClusterTest, Insert) {
  Eventlet e;
  cluster->insert_eventlet(e);
...
// 0-adc-valued eventlets will be a problem
...
TEST_F(ClusterTest, TimeNoOverlap) {
  ...
  // TODO: these are adjacent
  e.time = 6;
  cluster2.insert_eventlet(e);
  e.time = 12;
  cluster2.insert_eventlet(e);
  EXPECT_EQ(cluster->time_overlap(cluster2), 0);
}
