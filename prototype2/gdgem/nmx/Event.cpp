...
void Event::merge(PlaneNMX& cluster, uint8_t plane_id)
{
...
bool Event::time_overlap_thresh(const PlaneNMX& other, double thresh) const
{
  auto ovr = time_overlap(other);
  return (((ovr / other.time_span()) + (ovr / time_span())) > thresh);
}

double Event::time_overlap(const PlaneNMX& other) const
{
  auto latest_start = std::max(other.time_start, time_start());
  auto earliest_end = std::min(other.time_end, time_end());
  if (latest_start > earliest_end)
...
double Event::time() const { return time_start_; }
