...
public:
  ...
  inline uint16_t wire(uint8_t bus, uint16_t channel) {
    if (bus >= buses.size())
  ...
  inline uint16_t grid(uint8_t bus, uint16_t channel) {
    if (bus >= buses.size())
      throw std::runtime_error("Bus not defined");
    const auto &b = buses.at(bus);
  ...
  inline uint32_t x(uint8_t bus, uint16_t channel) {
    if (bus >= buses.size())
      throw std::runtime_error("Bus not defined");
    const auto &b = buses.at(bus);
  ...
  inline uint32_t y(uint8_t bus, uint16_t channel) {
    if (bus >= buses.size())
      throw std::runtime_error("Bus not defined");
    const auto &b = buses.at(bus);
  ...
  inline uint32_t z(uint8_t bus, uint16_t channel) {
    if (bus >= buses.size())
      throw std::runtime_error("Bus not defined");
    const auto &b = buses.at(bus);
