...
MesytecData::error MesytecData::parse(const Buffer& buffer, MgStats& stats) {
...
void MesytecEFU::parse(const std::vector<Buffer> &buffers,
                       EV42Serializer& serializer, MgStats& stats) {
  for (const auto &b : buffers) {
// Parse VMMR16
    vmmr16Parser.parse(b, stats);
...
MesytecEFU::MesytecEFU(std::shared_ptr<MgEFU> mg_efu,
                         bool spoof_ht, std::shared_ptr<MGHitFile> dump)

    : mgEfu(mg_efu)
    , dumpfile(dump) {
  vmmr16Parser.spoof_high_time(spoof_ht);
}

void MesytecEFU::set_geometry(ESSGeometry g)
{
  Geometry = g;
}

uint32_t MesytecEFU::getPixel() {
  if (!mgEfu)
    return 0;
