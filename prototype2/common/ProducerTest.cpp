...
TEST_F(ProducerTest, ConstructorOK) {
  ...
  ASSERT_EQ(prod.delivery_callback.stats.dr_errors, 0);
  ASSERT_EQ(prod.delivery_callback.stats.dr_noerrors, 0);
  ASSERT_EQ(prod.event_callback.stats.ev_errors, 0);
  ASSERT_EQ(prod.event_callback.stats.ev_others, 0);
}
