...
TEST_F(MBCAENBaseTest, DataReceive) {
  ...
  TestUDPServer Server(43126, Settings.DetectorPort, (unsigned char *)&pkt13_short[0], pkt13_short.size());
  Server.startPacketTransmission(1, 100);
  std::chrono::duration<std::int64_t, std::milli> SleepTime(200);
  std::this_thread::sleep_for(SleepTime);
  Readout.stopThreads();
  EXPECT_EQ(Readout.mystats.rx_packets, 1);
  EXPECT_EQ(Readout.mystats.rx_bytes, pkt13_short.size());
  EXPECT_EQ(Readout.mystats.rx_readouts, 66); // number of readouts in pkt13_short
}
