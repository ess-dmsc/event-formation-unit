...
TEST(MBEventBuilder__Test, TimeWindow) {
    ...
    for (int itest = 0; itest < 4; itest++) {
        ...
        // Set the clock-cycle number for the fist point. 64 bit for the for loop (see below)
...
TEST(MBEventBuilder__Test, TimeWindow_OverFlow) {
    ...
    // The clock-cycle is a 32-bit counter. At ESS it is planned to be reset before the maximum is reached.
    // This test is to ensure proper handling if for some reason the clock will not be reset.
    ...
    // Expect false while clock-cycle number is within window - event though the 32 bit clock counter has overflown.
...
TEST(MBEventBuilder__Test, ClusterTime) {
    ...
    // Test that the clock-cycle number of the cluster currently being processed is set properly.
    ...
    // This data point is outside the time-window, therefore the cluster currently stored is processed and
...
TEST(MBEventBuilder__Test, Adjacency) {
    ...
    // This test ensures that the event-builder checks the data-points for adjacency correctly.
    // Signals are expected to originate from adjacent wires or strips. Non-adjacent points may stem from pile-up,
    // and are therfore discarded.

    // Single points first. This tests that the adjacency test does not fail when the event consists of only
    // one data-point (wire or strip).
    // Cases when no wire or strips signals are present are implicitly tested here as well.
    ...
    // We only test wire points, since the same algorithm is used for both wire and strip.

    // Adjacent points
    uint8_t adjacent[3][3] = {
            { 3,  4,  5},
            {10, 11, 12},
            {29, 30, 31}
    };
    ...
    for (int i = 0; i < 3; i++) {
        ...
        // Test that the counter for adjacency rejected points is incremented properly.
        EXPECT_EQ(1, p.getNumberOfAdjacencyRejected());
    }

    // Test that the adjacency check works for cases with both wire and strip points.
    ...
    for (uint i = 0; i < 3; i++) {
        ...
        for (uint ii = 0; ii < 3; ii++) {
            ...
            // This will give two events where the wire points are non-adjacent and the strip points adjacent
