...
// The values are : time-window [clock-cycles], # wire channels, # strip
// channels
...
// The data expects standard configuration of the event-builder (see config
// above).
...
//
// Do not change these values unless you know what you are doing. If changed,
// change also
...
std::vector<uint> data{
    ...
    1, 33, 800, 2000, 1, 2, 34, 1500, 2010, 0,
    // Event # 3 - 3 data-points
    3, 35, 800, 3000, 1, 4, 36, 1700, 3010, 0, 5, 37, 500, 3020, 0,
    // Event # 4 - 4 data-points
    6, 38, 400, 4000, 1, 7, 39, 600, 4010, 0, 8, 40, 750, 4020, 0, 9, 41, 1800,
    4030, 0,
    // Event # 5 - 5 data-points
    10, 42, 1800, 5000, 1, 11, 43, 1000, 5010, 0, 12, 44, 500, 5020, 0, 13, 45,
    600, 5030, 0, 14, 46, 200, 5040, 0,
    // Event # 6 - 6 data-points
    15, 47, 1800, 6000, 1, 16, 48, 1000, 6010, 0, 17, 49, 500, 6020, 0, 18, 50,
    600, 6030, 0, 19, 51, 200, 6040, 0, 20, 52, 100, 6050, 0,
    // Final event - 1 data-point (After this event lastPoint() should be
    // called)
    21, 53, 1800, 7000, 1};

std::vector<double> validation_weighted{

    // This vector contains the expected values from the clustering process,
    // when the location method
    // is set to weighted average (default).
    // The ordering must not be changed!
    // These values must only be changed when data (above) is changed -
    ...
    // Event # 3
    static_cast<double>(3 * 800 + 4 * 1700 + 5 * 500) /
        static_cast<double>(800 + 1700 + 500),
    // Event # 4
    static_cast<double>(6 * 400 + 7 * 600 + 8 * 750 + 9 * 1800) /
        static_cast<double>(400 + 600 + 750 + 1800),
    // Event # 5
    static_cast<double>(10 * 1800 + 11 * 1000 + 12 * 500 + 13 * 600 +
                        14 * 200) /
        static_cast<double>(1800 + 1000 + 500 + 600 + 200),
    // Final event
