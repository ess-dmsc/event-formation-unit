...
public:
   ...
   * channel and a clock-cycle number. Each data point is added through this
   * function. The first sanity check on the
   * data is done here (See \subpage checks). The algorithm building the
   * clusters/events is described in
   * \subpage clustering.
   * Once a cluster is completed the position is calculated automatically and
   * the function returns true. Reception of
   * the true boolean should therefore trigger the readout of the calculated
   * position.
   *
   ...
   * stored cluster will be processed.
   */
  void lastPoint();

  /*! Returns the calculated wire position. The value is only updated when a
   * cluster/event is complete.
   * One should therefore only call this function when addDataPoint() returns
   * true.
   *
  ...
  /*! Set the ADC-threshold to accept data-points. ADC-values below a certain
   * value are considered to originate
   * from gammas and not neutrons. Gamma events are suppressed by a threshold in
   * the hardware. To increase this
  ...
  /*! Set the time-window for accepting data-points as belonging to the same
   * cluster (neutron-event. The time-stamps
   * of the data-points are in number of clock-cycles (since last reset).
   * Therefore the time-window must be specified
   * as the maximum number of clock-cycles after the first data-point in which a
   * point will be added to the cluster.
   *
   * @param m_time_window Cluster time-window.
   */
  void setTimeWindow(uint32_t time_window) { m_time_window = time_window; }

  /*! Toggle whether or not to use weighted average as position for wire and
   * strip.
   *
  ...
  /*! Set the number of wire channels in the cassette. Channels with higher
   * numbers than this will be assumed to
   * be strips channels. Channel numbers higher than the sum of wire and strip
  ...
  /*! Set the number of strip channels in the cassette. Channel numbers higher
   * than the sum of wire and strip
   * channels will be considered invalid.
   *
   * @param nchannels
   */
...
private:
  ...
  std::array<uint64_t, 6>
      m_2D_strips; /*!< 2D clusters. Number of strips fired. >5 strips ->
                      element 6 */
  ...
  /*! This function checks that signals originate from both wires and strips (if
   * present in the cluster) are adjacent.
   * Calls checkAdjacency() twice - once on the wire points and once on the
   * strip points.
  ...
  /*! This function checks whether points from either wires or strips are
   * adjacent.
   * Future implementations could include a mask of dead-channels as well as
