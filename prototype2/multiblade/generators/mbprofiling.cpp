...
int main(int argc, const char **argv) {
  ...
  try {
    ...
    TCLAP::ValueArg<std::string> ifileArg("f", "ifile", "File to analyze", true,
                                          "homer", "string");
    cmd.add(ifileArg);
    TCLAP::ValueArg<uint> nArg("n", "npasses", "Number of data passes ", false,
                               UINT_MAX, "integer");
    cmd.add(nArg);
  ...
  std::cout << "Number of times the data will be passed : " << npasses
            << std::endl;
  std::cout << "\n";
  ...
  while (ipass < npasses) {
    ...
    for (uint i = 0; i < entries.size(); i++) {
      bool cluster_complete =
          p.addDataPoint(entries[i].chan, entries[i].adc, entries[i].time);
      if (cluster_complete) {
  ...
  double_t avtime =
      static_cast<double>(acc_time.count()) / static_cast<double_t>(npoints);
  double_t evtime =
      static_cast<double>(acc_time.count()) / static_cast<double_t>(nevents);

  std::cout << "\nTime to load data [ns]           : " << loadtime.count()
            << std::endl;
  std::cout << "Average time per data-point [ns] : " << avtime << std::endl;
  std::cout << "Average time per event [ns]      : " << evtime << std::endl;
  std::cout << "Processing time [ns]             : " << acc_time.count()
            << std::endl;
  std::cout << "Total time [ns]                  : " << ttime.count()
            << std::endl;

  std::cout << "\nAccumulted wire  = " << std::setprecision(2) << accwire
            << std::endl;
  std::cout << "Accumulted strip = " << std::setprecision(2) << accstrip
            << std::endl;
  std::cout << "Accumulted time  = " << std::setprecision(2) << acctime
            << std::endl;
