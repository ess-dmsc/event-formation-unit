...
namespace SpscBuffer {
  ...
  public:
    CircularBuffer(std::int32_t Elements) : DataBuffer(new DataType[Elements]), DataQueue(Elements), EmptyQueue(Elements) {
...
#endif
      bool Success = DataQueue.try_enqueue(std::forward<ElementPtr<DataType>>(Element));
#ifndef NDEBUG
    ...
    bool tryPutEmpty(ElementPtr<DataType> &&Element) {
      ...
      DataType *TempPtr = Element.get();
#endif
      bool Success = EmptyQueue.try_enqueue(std::forward<ElementPtr<DataType>>(Element));
#ifndef NDEBUG
