...
void SetCLIArguments(CLI::App &parser, AdcSettingsStruct &AdcSettings) {
  ...
  parser.add_option("--mean_of_samples", CBFunc, "Only used when serializing data. Take the mean of # of samples (oversample) and serialize that mean.")->group("ADC Readout Options")->set_default_val("1");
