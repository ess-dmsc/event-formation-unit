...
struct fwdinfo_1_t FLATBUFFERS_FINAL_CLASS : private flatbuffers::Table {
  ...
  bool mutate_seq_data(uint64_t _seq_data) { return SetField(VT_SEQ_DATA, _seq_data); }
  uint64_t seq_fwd() const { return GetField<uint64_t>(VT_SEQ_FWD, 0); }
  bool mutate_seq_fwd(uint64_t _seq_fwd) { return SetField(VT_SEQ_FWD, _seq_fwd); }
  uint64_t ts_data() const { return GetField<uint64_t>(VT_TS_DATA, 0); }
  bool mutate_ts_data(uint64_t _ts_data) { return SetField(VT_TS_DATA, _ts_data); }
  uint64_t ts_fwd() const { return GetField<uint64_t>(VT_TS_FWD, 0); }
  bool mutate_ts_fwd(uint64_t _ts_fwd) { return SetField(VT_TS_FWD, _ts_fwd); }
  uint32_t fwdix() const { return GetField<uint32_t>(VT_FWDIX, 0); }
  bool mutate_fwdix(uint32_t _fwdix) { return SetField(VT_FWDIX, _fwdix); }
  uint64_t teamid() const { return GetField<uint64_t>(VT_TEAMID, 0); }
  bool mutate_teamid(uint64_t _teamid) { return SetField(VT_TEAMID, _teamid); }
