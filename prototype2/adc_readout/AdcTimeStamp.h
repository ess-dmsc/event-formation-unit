...
/// @brief Maps to the timestamps as received from (and used by) the ADC hardware.
struct RawTimeStamp {
  ...
  /// @note The epoch might change in the future.
  std::uint32_t Seconds{0};
  
  /// @brief Number of clock cycles of the timing hardware clock divided by two.
  // This value resets to zero once every second and has a maximum value of ≈44 million.
  // @note It is possible that in future systems, the clock frequency will not be 44 MHz.
  ...
  /// @brief Convert from raw timestamp (seconds plus fractional seconds) into nanoseconds since epoch.
  /// This version uses floating point math to do the conversion and is likely to be slower than RawTimeStamp::GetTimeStampNSFast() on machines with a slow floating point unit.
  /// @return Number of nanoseconds since epoch (currently EPICS epoch).
  std::uint64_t GetTimeStampNS() const;

  /// @brief Convert from raw timestamp (seconds plus fractional seconds) into nanoseconds since epoch.
  /// This version uses integer math to do the conversion and is likely to as fast as or slower than RawTimeStamp::GetTimeStampNS() on CPUs that have a fast floating point unit.
  /// @return Number of nanoseconds since epoch (currently EPICS epoch).
