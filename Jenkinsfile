    ...
    if (container.key == clangformat_os) {
        ...
        try {
                ...
                container.sh """
                                cd ${project}
                                cppcheck --version
                                cppcheck --library=googletest -Isrc -itest --enable=all --suppressions=--inconclusive --template="{file},{line},{severity},{id},{message}" --xml --xml-version=2 ./src 2> ${test_output}
                            """
